...
#include <stdlib.h>
#include <ctype.h>
#define MS_PER_ANIMATION_CYCLE 8000
#define NUMPOINTS 12
#define PI 3.14
...
Animate( )
{
	...
	ms %= MS_PER_ANIMATION_CYCLE;
	Time = (float)ms / (float)(MS_PER_ANIMATION_CYCLE);
...
Display( )
{
	...
	int curves = 90;
	int z = ((sin(2 * PI*Time) / 6) + 1.55);
	for (int j = 0; j < curves; j++) {
		float angle = 360. * j / curves;
		struct Curve curve;
		struct Point p0 = { 0., 0., 0. };
		curve.p0 = p0;
		struct Point p1 = { 1 * cos(angle), 1.,1 * sin(angle) };
		curve.p1 = p1;
		struct Point p2 = { ((sin(Time *2 * PI) / 3) + 1.55) * cos(angle / 3) * 1, ((sin(Time *2 * PI) / 3) + 1.55) * 1,1 * sin(3 * angle)*((sin(Time *2 * PI) / 3) + 1.55) };
		curve.p2 = p2;
		struct Point p3 = { cos(angle / 1) * 2, 2. - ((sin(Time*2 * PI) / 3) + 1.55) * 2,2 * sin(angle / 1) };
		curve.p3 = p3;
		...
		RotateX(&p0, 270., 0., 0., 0.);
		RotateX(&p1, 270., 0., 0., 0.);
		RotateX(&p2, 270., 0., 0., 0.);
		RotateX(&p3, 270., 0., 0., 0.);
		curve.r = 1;
		curve.g = 0;
		curve.b = 0;
		DrawCurve(&curve);
	...
	for (int i = 0; i < curves; i++) {
		...
		curve.p2 = p2;
	struct Point p3 = { sin(angle) / 4, -2., cos(angle) / 4 };
		curve.p3 = p3;
		RotateY(&p1, Time, 0., 0., 0.);
    	curve.r = 0.0;
		curve.g = 0.3;
		curve.b = 0.;
		DrawCurve(&curve);
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Omkar Thakur: Project 06" );
