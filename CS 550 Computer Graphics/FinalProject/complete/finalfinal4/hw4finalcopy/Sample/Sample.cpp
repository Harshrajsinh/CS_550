...
Animate( )
{
	...
	Time2 = (float)ms / (float)MS_PER_CYCLE + Time2;		// [0.,1.)
...
Display()
{
	...
	//gluLookAt(-2.,6.2, 7.5, 0.,-10., 0., 0., 1., 0.); //perfect moon


	if (cam5)
	{

		//gluLookAt(-2., 6.2, 7.5, 0., -10., 0., 0., 1., 0.);
	gluLookAt(-2, 3, 2, 0., 2., 0., 0., 1., 0.);//view from sun
	}
	//original

	if (cam1)
	{
		//gluLookAt(-3.5, 2., 5., 9., 7., 0., 0., 1., 0.);
		gluLookAt(-3, -3., 4., -12., -10., 3., 0., 1., 0.);//oregon to the world
		//gluLookAt(-1, -1., 3., 4., -2., 0., 0., 1., 0.);
		//gluLookAt(-2, -2., 4., 4., -4., 0., 1., 5., 0.);
		

	}
	//gluLookAt(20 * cos(1 * M_PI * spin/2 ), -3, -10, 0., 0., 0., 0., 1., 0.);   //reflection

	if (cam2)
	{
		gluLookAt(-2, 8, 8, 0., 2., 0., 0., 1., 0.); //moon looking to oregon
		
		//gluLookAt(-2, 1, 1, 0., 0., 0., 4., 1., -4.);
	}


	if (cam3)
	{
		gluLookAt(15., 4, 10 * sin(1 * M_PI*Time2 / 200) * 5, 0., 0., 0., 0., 1., 0.);// galaxy  camera
		//gluLookAt(15., 4, 20 * cos(1 * M_PI/9*Time / 8) * 5, 0., 0., 0., 0., 1., 0.);
	}
	...
	glEnable(GL_TEXTURE_2D);     //sun
	glShadeModel(GL_SMOOTH);
	SetMaterial(1, 1, 1, 5);
	glCallList(SunList);
	glTranslatef(-14, 5, 10);
	MjbSphere(5., 50, 50);
	...
	glPushMatrix();
	if (Light2On) {
		SetPointLight(GL_LIGHT1, -14, 5, 10, 1, 1, 0);    //spot light
		//SetSpotLight(GL_LIGHT1, -14, 5, 10, 0, 0, 0,1, 1, 0);
	...
	glPushMatrix();
		...
		SetSpotLight(GL_LIGHT2, -14, 5, 10, 0, 0, 0, 1, 1, 0);
...
//	glPopMatrix();
	...
	glPushMatrix();
	glColor3f(1, 0, 1);
	glTranslatef(1 * sin(1 * M_PI / 2 * Time / 3), -6, -28);
	SetMaterial(1, 0, 1, 0.);
	MjbSphere(0.2, 10, 10);
	glPopMatrix();
	...
	glPushMatrix();
	glColor3f(1, 0, 0);
	glTranslatef(1 * sin(1 * M_PI / 2 * Time / 3), 3, -28);
	SetMaterial(1, 0, 1, 0.);
	MjbSphere(0.2, 10, 10);
	glPopMatrix();
	...
	//glTranslatef(-2, 7 * cos(4 * M_PI / 9 * Time2 / 13), 3);
	glTranslatef(-3, cos(2 * 3.14*Time2 / 30), (sin(1 * 3.14*Time)));
	glRotatef(spin * 10, 1, 0, 2);
	glutSolidCube(0.12);
    glPopMatrix();
	...
	glPushMatrix();
	SetMaterial(1, 0, 0, 10);
	//glTranslatef(7 * cos(4 * M_PI / 9 * Time2 / 13),-4 , 3);
	glTranslatef((sin(1 * 3.14*Time)), cos(1 * 3.14*Time2 / 30), 4);
	glRotatef(spin * 10, 1, 0, 2);
	glutSolidCube(0.12);
	glPopMatrix();
	...
	glPushMatrix();
	if (Light1On) {
		SetSpotLight(GL_LIGHT2, 1, 0, 0, 10., 10., 10.,1,0,0); //red  light    
	}
	else {
		glDisable(GL_LIGHT2);
	}
	...
	glTranslatef(4, 2, 2 * sin (1*M_PI*Time2/200)*5);// shuttle goes in one direction
	//glTranslatef(4, 2, 2 * sin(1 * M_PI*Time/50) * 5);
	//glTranslatef(-7, cos(2 * 3.142*Time2/30), (sin(2 * 3.142*Time)));
	//glTranslatef(0, 0, -25);
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Omkar Thakur CS 550 Final Project" );
...
InitLists( )
{
	...
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	glGenTextures(1, &tex4); // assign binding “handles”
	glGenTextures(1, &tex5);

	glBindTexture(GL_TEXTURE_2D, tex4); // make tex0 texture current
										// and set its parameters
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
